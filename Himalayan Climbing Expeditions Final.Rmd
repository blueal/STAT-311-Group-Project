...
Variables to use
highpoint_metres = dead, season, party_size


```{r model3,echo = FALSE}
m3_df <- expeditions %>% mutate(party_size = members, hired_staff)
...
graph3 <- m3_df %>% ggplot(mapping = aes(x = party_size,y = highpoint_metres)) +
  geom_point() +
  geom_smooth(method = "loess", formula = y ~ x)
...
m3_df_split <- initial_split(m3_df, prop = 0.8)
